...
```bash
pi install git:github.com/dheerapat/pi-keb
```
...
3. **Summary:** The LLM writes a 200–400 word summary via `keb_write_summary`. A **deterministic footer** is appended — concept links are extracted from the body and listed as `[[concept/...]]` references
4. **Concepts:** The LLM creates new topics via `keb_write_concept` or extends existing ones via `keb_update_concept`:
   - `keb_write_concept` — creates a new concept with an explicit sources list
...
5. **Index:** The LLM calls `keb_update_index` which **filters entries against disk** before writing — any slug the LLM invented that doesn't correspond to a real file is silently dropped
